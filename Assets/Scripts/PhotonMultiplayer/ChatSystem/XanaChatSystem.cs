...
public class XanaChatSystem : MonoBehaviour, IChatClientListener
{
    ...
    public void Connect()
    {
        ...
        Debug.LogError("this user name :- " + this.UserName);
    ...
    public void OnConnected()
    {
        ...
        if (this.ChannelsToJoinOnConnect != null)
        {
            ...
            {
                this.ChannelsToJoinOnConnect = XanaConstants.xanaConstants.EnviornmentName;// FeedEventPrefab
            }
            else
            {
    ...
    public void OnGetMessages(string channelName, string[] senders, object[] messages)
    {
        ...
        //    }
        //}
      //if(XanaChatLand.activeInHierarchy)
        {
            Debug.Log("Chat visible==="+ channelName);
            if (channelName.Equals(this.selectedChannelName))
            {
                // update text
                this.ShowChannel(this.selectedChannelName);
                Debug.Log("Chat visible 4===" + GameManager.currentLanguage);
                //if (GameManager.currentLanguage == "en")
                //{
                    Debug.Log("Chat visible 5===" + GameManager.currentLanguage);
                    if (senders[senders.Length - 1].Length > 12)
                    {
                        Debug.Log("Chat visible 1===" + channelName);
                        this.CurrentChannelText.text =
                        "<b>" + senders[senders.Length - 1].Substring(0, 12) + "...</b>" + " : " + (string)messages[messages.Length - 1] + "\n" + this.CurrentChannelText.text;
                    }
                    else
                    {
                        Debug.Log("Chat visible 2===" + channelName);
                        this.CurrentChannelText.text =
                       "<b>" + senders[senders.Length - 1].Substring(0, senders[senders.Length - 1].Length) + "</b>" + " : " + (string)messages[messages.Length - 1] + "\n" + this.CurrentChannelText.text;
                    }
                ...
                //if (GameManager.currentLanguage == "ja")
                //{
                //    Debug.Log("Chat visible 3===" + channelName);
                //    //this.CurrentChannelText.text +=
                //    //    senders[senders.Length - 1] + " : " + (string)messages[messages.Length - 1] + "\n";
                //    // commented by Usman Aslam
                //    string lastMessage = (string)messages[messages.Length - 1];
