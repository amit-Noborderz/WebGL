...
namespace MD_Plugin
{
    ...
    public class MDM_SculptingLite : MonoBehaviour
    {
        ...
        public List<Vector3[]> terrainRedoVerticesPositions = new List<Vector3[]>();
        int redoIndex = 0;
        ...
        private void SS_Funct_ManageControls(RaycastHit hit)
        {
            ...
            {
                if (Internal_GetControlInput(SS_SculptingLowerInput) && SS_UseRaiseFunct && resetTerrain)
                {
                    ...
                    if (!SS_UpdateColliderAfterRelease)
                    {
                        ...
                        LevelTerrainWhenReset();
                    }

                }
                else if (Internal_GetControlInput(SS_SculptingLowerInput) && SS_UseLowerFunct && !resetTerrain)
                {
                    // terainBrush.ChangeBrush(4);
                    SS_Funct_DoSculpting(hit.point, SS_BrushProjection.transform.forward, SS_BrushSize, SS_BrushStrength, SS_State_Internal.Lower);
                    if (!SS_UpdateColliderAfterRelease)
                        SS_Funct_RefreshMeshCollider();
                }
            ...
            while (i < workingVertices.Length)
            {
                if (Vector3.Distance(transform.TransformPoint(workingVertices[i]), WorldPoint) < Radius)
                {
                    ...
                    else
                    {
                        ...
                        workingVertices[i] = transform.InverseTransformPoint(new Vector3(origin.x, Mathf.Clamp(origin.y, -10, 0f), origin.z));
                    }
                }
                i++;
            }
        ...
        private void LevelTerrainWhenReset()
        {
            meshVertices = meshFilter.sharedMesh.vertices;
            for (int i = 0; i < meshVertices.Length; i++)
            {
                if (meshVertices[i].y > 0) meshVertices[i] = new Vector3(meshVertices[i].x, 0, meshVertices[i].z);
            }
        ...
        public void SS_Funct_RecordToHistory()
        {
            ...
            // print(historyRecords.Count + " <color=red>HISTORY RECORD COUNT</color>");
        ...
        private Vector3[] GetCurrentMeshVerticesPosition()
        {
        ...
        /// Default undo method
        /// </summary>
        [ContextMenu("UNDO MESH HISTORY")]
        public void SS_Funct_Undo()
        {
            terrainRedoVerticesPositions.Add(GetCurrentMeshVerticesPosition());
            redoIndex++;
            SS_Funct_Undo(-1);
            print("UNDO THE CHANGES");
        }



        public void SS_Funct_Redo()
        {
            SS_Funct_Redo(1);
            print("REDDO THE CHANGES");
        }
        private void SS_Funct_Redo(int dummy)
        {
            // SS_Funct_RecordToHistory();
            ...
            if (historyRecords.Count > SS_MaxHistoryRecords)
                historyRecords.RemoveAt(0);
            HistoryRecords h = new HistoryRecords() { historyNotes = "History " + historyRecords.Count.ToString(), VertexPositions = new Vector3[originVertices.Length] };
            h.VertexPositions = GetCurrentMeshVerticesPosition();
            historyRecords.Add(h);

            Vector3[] tempVerticesPosition = terrainRedoVerticesPositions[terrainRedoVerticesPositions.Count - 1];
            meshFilter.sharedMesh.SetVertices(tempVerticesPosition);
            workingVertices = tempVerticesPosition;
            ...
            print(terrainRedoVerticesPositions.Count + " TERRAIN REDO LIST COUNT");
            
            terrainRedoVerticesPositions.RemoveAt(redoIndex - 1);
            redoIndex--;
        }
